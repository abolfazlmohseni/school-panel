...
$sql .= " ORDER BY $order_column $direction, id DESC";
    ...
    <div class="min-h-screen lg:mr-64">
        <div class="p-4 sm:p-6 lg:p-8">
            <div class="w-full">
                <!-- Header -->
                <div class="mb-6">
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">دانش‌آموزان کلاس
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                    <div class="p-4 sm:p-6">
                        ...
                        <form method="GET" class="mb-6 flex flex-col sm:flex-row gap-3">
                            ...
                            <input type="text" name="search" placeholder="جستجو بر اساس نام یا کدملی ..."
                            ...
                            <table class="w-full">
                                <thead class="bg-gray-50 border-b-2 border-gray-200">
                                    <tr>
                                        ...
                                        <th
                                            class="px-4 py-3 text-right text-xs sm:text-sm font-semibold text-gray-700 whitespace-nowrap">
                                            <a href="?id=<?= $class_id ?>&search=<?= urlencode($search) ?>&order=first_name&dir=<?= ($order == 'first_name' && $direction == 'asc') ? 'desc' : 'asc' ?>"
                                        ...
                                        <th
                                            class="px-4 py-3 text-right text-xs sm:text-sm font-semibold text-gray-700 whitespace-nowrap">
                                            <a href="?id=<?= $class_id ?>&search=<?= urlencode($search) ?>&order=last_name&dir=<?= ($order == 'last_name' && $direction == 'asc') ? 'desc' : 'asc' ?>"
                                                class=" flex items-center gap-1">
                                                نام خانوادگی
                                        ...
                                        <th
                                            class="px-4 py-3 text-right text-xs sm:text-sm font-semibold text-gray-700 whitespace-nowrap">
                                            <a href="?id=<?= $class_id ?>&search=<?= urlencode($search) ?>&order=national_code&dir=<?= ($order == 'national_code' && $direction == 'asc') ? 'desc' : 'asc' ?>"
                                                class=" flex items-center gap-1">
                                                کد ملی
                                                <?php if ($order == 'national_code'): ?>
                                                    <span class="text-blue-600">
                                                        <?= $direction == 'asc' ? '↑' : '↓' ?>
                                                    </span>
                                                <?php endif; ?>
                                                ...
                                                <td class="px-4 py-3 text-center whitespace-nowrap">
                                                    <div class="flex gap-2 justify-center flex-wrap">
                                                        ...
                                                        <a href="student_delete.php?id=<?= $row['id'] ?>"
                                                            onclick="return confirm('آیا مطمئن هستید این دانش‌آموز حذف شود؟')"
                ...
                <div class="mt-6"><a href="classes.php"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-300 transition-colors duration-200 text-sm sm:text-base">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewbox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7">
                            </path>
                        </svg> بازگشت به لیست کلاس‌ها </a>
                </div>
            </div>
        </div>
    </div>
